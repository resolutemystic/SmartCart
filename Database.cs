...
namespace SmartCart
{
    ...
    {
        static string dbPath = Path.Combine(FileSystem.AppDataDirectory, "grocery.db");
            ...
            if (!File.Exists(dbPath))
            {
                ...
                cmd.CommandText = @"
                    ...
                    INSERT INTO GroceryItems (itemID, name, category) VALUES
                        ...
                        (36,'White Bread Loaf',6), (37,'Sourdough Bread Loaf',6), 
                        (38,'Whole Wheat Bread Loaf',6), (39,'Cherry Pie',6), 
                        (40,'Apple Pie',6), (41,'Berry Pie',6), (42,'Muffins',6), 
                        (43,'Cake',6), (44,'Potato Chips',8), (45,'Tortilla Chips',8), 
                        (46,'Tortillas',6), (47,'Popcorn',8), (48,'Fruit Snacks',8), 
                        (49,'Trail Mix',8), (50,'Mixed Nuts',8), (51,'Hard Candy',8), 
                        (52,'Sour Candy',8), (53,'Gummy Candy',8), (54,'Bouillon Powder',9), 
                        (55,'Beef Broth',9), (56,'Bone Broth',9), (57,'Chicken Broth',9), (58,'Mushrooms',5);
        ...
        public static void RefreshDatabase()
        {
            try
            {
                ...
                // Force garbage collection to ensure file is not locked
                GC.Collect();
                ...
                // Wait briefly to ensure delete is finalized (important on Android)
                Thread.Sleep(250); // <-- Add this line
                InitializeDatabase();

                // Clear in-memory list
                GroceryList.currentList = new List<GroceryItem>();
            }
        ...
        public static void DeleteEntry(int entryID)
        {
            ...
            connection.Open();
            var cmd = connection.CreateCommand();
            cmd.CommandText = $"DELETE FROM GroceryList WHERE entryID = {entryID}";
            cmd.ExecuteNonQuery();
        }
        ...
        public static void DeleteCheckedItems()
        {
            ...
            connection.Open();
            var cmd = connection.CreateCommand();
            cmd.CommandText = "DELETE FROM GroceryList WHERE isChecked = 1";
            cmd.ExecuteNonQuery();
        }
        ...
        public static void UpdateQuantity(int entryID, int quantity)
        {
            ...
            connection.Open();
            var cmd = connection.CreateCommand();
            cmd.CommandText = $"UPDATE GroceryList SET quantity = {quantity} WHERE entryID = {entryID}";
            cmd.ExecuteNonQuery();
        }
        ...
        public static void IncreaseQuantity(int entryID, int quantity)
        {
            ...
            connection.Open();
            var cmd = connection.CreateCommand();
            cmd.CommandText = $"UPDATE GroceryList SET quantity = quantity + {quantity} WHERE entryID = {entryID}";
            cmd.ExecuteNonQuery();
        }
        ...
        public static void UpdatePriority(int entryID, int priority)
        {
            ...
            connection.Open();
            var cmd = connection.CreateCommand();
            cmd.CommandText = $"UPDATE GroceryList SET priority = {priority} WHERE entryID = {entryID}";
            cmd.ExecuteNonQuery();
        }
        ...
        private static void BuildCategories()
        {
            ...
            while (reader.Read())
                categoryDict[reader.GetString(1)] = reader.GetInt32(0);
        }
        ...
        private static void BuildGroceryItems()
        {
            ...
            var reader = cmd.ExecuteReader();
            while (reader.Read())
                groceryItemDict[reader.GetString(1)] = reader.GetInt32(0);
        }
        ...
        private static void BuildPriorities()
        {
            ...
            var reader = cmd.ExecuteReader();
            while (reader.Read())
                priorityDict[reader.GetString(1)] = reader.GetInt32(0);
        }
        ...
        public static void UpdateCategorizedItems(int categoryID)
        {
            ...
            connection.Open();
            var cmd = connection.CreateCommand();
            cmd.CommandText = categoryID == 0
                ? "SELECT itemID, name FROM GroceryItems"
                : $"SELECT itemID, name FROM GroceryItems WHERE category = {categoryID}";
            var reader = cmd.ExecuteReader();
            while (reader.Read())
                categorizedItemDict[reader.GetString(1)] = reader.GetInt32(0);
        }
    }
}
